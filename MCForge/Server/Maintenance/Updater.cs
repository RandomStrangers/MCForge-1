...
namespace MCForge {
    ...
    public static class Updater {
        ...
        public const string UploadsURL = "https://github.com/RandomStrangers/MCForge/blob/master/Uploads";
        
        const string CurrentVersionURL = BaseURL + "Uploads/current_version.txt";
        #if TEN_BIT_BLOCKS
        const string dllURL = BaseURL + "Uploads/MCForge_infid.dll?raw=true";
        #else
        const string dllURL = BaseURL + "Uploads/MCForge_.dll?raw=true";
        #endif
        const string changelogURL = BaseURL + "Changelog.txt";
        const string guiURL = BaseURL + "Uploads/MCForge.exe?raw=true";
        const string cliURL = BaseURL + "Uploads/MCForgeCLI.exe?raw=true";
        ...
        public static void PerformUpdate() {
            try {
                try {
                    DeleteFiles("Changelog.txt", "MCForge_.update", "MCForge.update", "MCForgeCLI.update",
                                "prev_MCForge_.dll", "prev_MCForge.exe", "prev_MCForgeCLI.exe");
                } catch {
                }
                
                WebClient client = HttpUtil.CreateWebClient();
                client.DownloadFile(dllURL, "MCForge_.update");
                client.DownloadFile(guiURL, "MCForge.update");
                client.DownloadFile(cliURL, "MCForgeCLI.update");
                ...
                AtomicIO.TryMove("MCForge_.dll", "prev_MCForge_.dll");
                AtomicIO.TryMove("MCForge.exe", "prev_MCForge.exe");
                AtomicIO.TryMove("MCForgeCLI.exe", "prev_MCForgeCLI.exe");
