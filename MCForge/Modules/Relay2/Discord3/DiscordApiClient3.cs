...
using MCForge.Config;
using MCForge.Network;

namespace MCForge.Modules.Relay2.Discord3
{
    /// <summary> Represents an abstract Discord API message </summary>
    public abstract class DiscordApiMessage3
    {
        ...
        public virtual bool CombineWith(DiscordApiMessage3 prior) { return false; }
    }
    
    /// <summary> Message for sending text to a channel </summary>
    public class ChannelSendMessage : DiscordApiMessage3
    {
        ...
        public override bool CombineWith(DiscordApiMessage3 prior) {
    ...
    public class ChannelSendEmbed : DiscordApiMessage3
    {
        public string Title;
    ...
    public sealed class DiscordApiClient3 : RelayBotSender2<DiscordApiMessage3>
    {
        public string Token;
        const string host = "https://discord.com/api/v8";
        
        DiscordApiMessage3 GetNextRequest() {
            if (requests.Count == 0) return null;
            DiscordApiMessage3 first = requests.Dequeue();
            
            // try to combine messages to minimise API calls
            while (requests.Count > 0) {
                DiscordApiMessage3 next = requests.Peek();
        ...
        static void LogError(Exception ex, DiscordApiMessage3 msg) {
